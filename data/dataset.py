...
class ImageLoader:
    ...
    def __call__(self, img):
        try:
            img = Image.open(ospj(self.root_dir, img)).convert(
                "RGB"
            )  # We don't want alpha
...
class CompositionDataset(Dataset):
    ...
    def __init__(
        ...
        self.feat_dim = (
            512 if "resnet18" in model else 2048
        )  # todo, unify this with models
        ...
        }  # all pairs [1962], for val in training
        ...
        if train_only and self.phase == "train":
            ...
            print(
                "  Using all pairs as classification classes during {} process".format(
                    self.phase
                )
            )
            self.pair2idx = {
        ...
        for _obj in self.objs:
            candidates = [
                attr
                for (_, attr, obj) in self.train_data + self.test_data
                if obj == _obj
            ]
    ...
    def generate_features(self, out_file, model):
        ...
        transform = dataset_transform(
            "test", self.norm_family
        )  # Do not use any image augmentation, because we have a trained image backbone
        ...
        image_files = []
        for chunk in tqdm(
            chunks(files_all, 1024),
            total=len(files_all) // 1024,
    ...
    def __getitem__(self, index):
        ...
        if not self.update_image_features:
            if "mit-states" in self.args.dataset:
                ...
                pair = pair.replace("_", " ")
                image = pair + "/" + img
            elif self.args.dataset.startswith("vg") or self.args.dataset.startswith(
                "paco"
            ):
