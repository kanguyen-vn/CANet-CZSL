...
def main():
    ...
    args.dataset = "mit-states+paco"  # Choose from ut-zap50k | mit-states | cgqa
    ...
    if "+" in args.dataset:
        generate_combined_dataset(args.dataset, args.data_root)
    ...
    for epoch in range(args.max_epochs):
        print(
            "Epoch {} | Best Attr: {:.2f}% | Best Obj: {:.2f}% | Best Seen: {:.2f}% | Best Unseen: {:.2f}% | Best HM: {:.2f}% | Best AUC: {:.2f} | Best Epoch: {:.0f}".format(
                epoch + 1,
                best_attr * 100,
                best_obj * 100,
                best_seen * 100,
...
def train_normal(args, epoch, image_extractor, model, trainloader, optimizer, writer):
    ...
    for idx, data in enumerate(trainloader):
        ...
        trainloader.set_description(
            desc="|--Training | Batch Loss: {:.4f}".format(loss.item())
        )
        if loss == None:
            trainloader.close()
            return
...
def test(args, epoch, image_extractor, model, testloader, evaluator, logpath):
    ...
    def save_checkpoint(filename):
        state = {"epoch": epoch + 1, "AUC": stats["AUC"]}
        ...
        torch.save(
            ...
            ospj(
                ...
                "logs",
                args.dataset,
                "ckpt_{}_{}.t7".format(filename, args.dataset),
            ),
        )
    ...
    for k in all_pred[0].keys():
        all_pred_dict[k] = torch.cat(
            [all_pred[i][k].cpu() for i in range(len(all_pred))]
        )
    del all_pred

    # Calculate best unseen accuracy
    results = evaluator.score_model(
        all_pred_dict, all_obj_gt, bias=args.bias, topk=args.topk
    )
    stats = evaluator.evaluate_predictions(
        results, all_attr_gt, all_obj_gt, all_pair_gt, all_pred_dict, topk=args.topk
    )
    ...
    AUC = stats["AUC"]
    print(
        "|----Test Finished: Attr Acc: {:.2f}% | Obj Acc: {:.2f}% | Seen Acc: {:.2f}% | Unseen Acc: {:.2f}% | HM: {:.2f}% | AUC: {:.2f}".format(
            attr_acc * 100,
            obj_acc * 100,
            seen_acc * 100,
            unseen_acc * 100,
