...
def load_word_embeddings(vocab, args):
    if args.emb_type == 'glove':
        ...
        raise ValueError('  Invalid embedding. You have to choose an existing embedding type. Exit.')
    args.emb_dim = embeds.shape[1]
    return embeds
...
def load_fasttext_embeddings(vocab):
    ...
    ft = fasttext.load_model(DATA_FOLDER+'fasttext/cc.en.300.bin')
    embeds = []
    for k in vocab:
        if '_' in k:
            ks = k.split('_')
